...
export default function SongCard({ songId, handleClose }) {
  ...
  useEffect(() => {
    fetch(`http://${config.server_host}:${config.server_port}/song/${songId}`)
      ...
      .then(res => {
        setSongData(res)
        fetch(`http://${config.server_host}:${config.server_port}/album/${res.album_id}`)
          .then(res => res.json())
            ...
            barRadar
              ? (
                ...
                <ResponsiveContainer height={250}>
                  ...
                  <RadarChart
                    outerRadius={90} width={730} height={250} data={chartData}
                  >
                    <PolarGrid/>
                    <PolarAngleAxis dataKey='name'/>
                    <PolarRadiusAxis angle={30} domain={[0,1]}/>
                    <Radar name='Me' dataKey='value' stroke='#8884d8' fill='#8884d8' fillOpacity={0.6}/>
                  </RadarChart>
                </ResponsiveContainer>
              )
          }
